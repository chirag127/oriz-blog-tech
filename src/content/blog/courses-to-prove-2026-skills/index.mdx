...
| RAG / vector DB | Fundamentals of AI Agents Using RAG and LangChain (IBM); Generative AI Engineering with LLMs (IBM specialization) | Hybrid RAG (dense+BM25+RRF+rerank), vector DB, eval numbers (Recall@k/MRR/nDCG) — [rag-lens](https://github.com/chirag127/rag-lens) |
...
| Go / systems | Programming with Google Go (UC Irvine); Go Essentials: Concurrency, gRPC & More (Packt) | gRPC + Postgres + Redis microservice with metrics, caching, graceful shutdown — [go-vault](https://github.com/chirag127/go-vault) |
